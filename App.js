...
export default function App() {
  const [listeEleve,setListeEleve] = useState([{id_etudiant: "00", nom:"Aucun étudiant sélectionné", session: "1", cours: []},
...
const [confirmeSelection, setConfirmeSelection] = useState("Selectioner un eleve");
...
function trouverIndex(value){
 ...
 let numeroEleve = listeEleve.findIndex((element) => element.id_etudiant == value);
...
function verifierCourValide(cour){
  ...
  if(listeEleve[etudiant].cours.length < 5 && !listeEleve[etudiant].cours.includes(cour)){
...
setNouveauCour("");
  ...
  return (
    <View style={styles.container}>
      ...
      <Text>Id:</Text>
      <TextInput onChangeText={(value)=>{trouverIndex(value)}}/>
      <Text>{eleve.nom}</Text>
      <Pressable style={styles.button} onPress={confirmerSelection}><Text>Choisir etudiant</Text></Pressable>
      <Text>{confirmeSelection}</Text>
      </View>
      <View>
        <Text>Session : {session}</Text>
        <TextInput placeholder='Changer session' onChangeText={(value)=> setSession(value)}/>
        <Text>Ajout un cours</Text>
        <TextInput onChangeText={(value)=>{ajoutCour(value)}}/>
        <Pressable style={styles.button} onPress={enregistrer}><Text>Enregistrer</Text></Pressable>
        <Pressable style={styles.button} onPress={()=> {alert(listeEleve[etudiant].cours)}}><Text>Afficher</Text></Pressable>
      </View>
